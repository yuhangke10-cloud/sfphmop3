(() => {
  ...
  const controls = Object.fromEntries(['monitorTab','eventTab','parameterTab','theme','mapModeGroup','parameterQuery','parameterBack'].map(id => [id, document.getElementById(id)]));
  ...
  function render(){
    ...
    setVisible('mapModeGroup',monitoring);
  ...
  controls.mapModeGroup.addEventListener('click',event=>{
    const rect=controls.mapModeGroup.getBoundingClientRect();
    const x=event.clientX-rect.left;
    const ratio=rect.width?x/rect.width:0;
    state.mode=ratio<113/314?'satellite':ratio<(113+88)/314?'line':'network';
    render();
  });
